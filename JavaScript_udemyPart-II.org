...
:PROPERTIES:
:CUSTOM_ID: javascript-fundamentals-part-2
:END:
...
:PROPERTIES:
:CUSTOM_ID: function-declarations-vs.-expressions
:END:
#+begin_src js
...
:PROPERTIES:
:CUSTOM_ID: functions-calling-other-functions
:END:
#+begin_src js
...
:PROPERTIES:
:CUSTOM_ID: introduction-to-arrays
:END:
#+begin_src js
...
:PROPERTIES:
:CUSTOM_ID: basic-array-operations-methods
:END:
#+begin_src js
const friends = ['Al-Farabi', 'Al-Ghazali', 'Al- Razi'];
...
:PROPERTIES:
:CUSTOM_ID: dot-vs.-bracket-notation
:END:
#+begin_src js
const xoraus = {
  firstName: 'Jordan',
...
:PROPERTIES:
:CUSTOM_ID: iteration-the-for-loop
:END:
#+begin_src js
...
:PROPERTIES:
:CUSTOM_ID: looping-arrays-breaking-and-continuing
:END:
#+begin_src js
const jonas = [
  'Jonas',
...
:END:
Let's improve Steven's tip calculator even more, this time using
loops! 1. Create an array 'bills' containing all 10 test bill values 2.
Create empty arrays for the tips and the totals ('tips' and 'totals') 3.
Use the 'calcTip' function we wrote before (no need to repeat) to
